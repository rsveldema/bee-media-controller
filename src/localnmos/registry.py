...
class NMOSServiceListener(ServiceListener):
    ...
    def _process_service_info(self, info: ServiceInfo, service_type: str, added: bool = True):
...
class NMOSRegistry:
    ...
    def __init__(self, device_added_callback: Optional[Callable] = None,
    ...
    async def start(self):
        ...
        if Zeroconf is None:
            logger.error("zeroconf library not available. Install with: pip install zeroconf")
            return
    ...
    def _on_device_added(self, device: NMOSDevice):
        ...
        logger.info(f"Device discovered: {device.name} at {device.address}:{device.port}")
        self.devices[device.device_id] = device
        
        if self.device_added_callback:
            # Schedule callback in event loop
            try:
                asyncio.create_task(self._call_callback(self.device_added_callback, device))
            except RuntimeError:
    ...
    def _on_device_removed(self, device: NMOSDevice):
        ...
        if self.device_removed_callback:
            try:
                asyncio.create_task(self._call_callback(self.device_removed_callback, device))
            except RuntimeError:
    ...
    async def query_device(self, device: NMOSDevice, endpoint: str = "/self") -> Optional[Dict]:
        """
