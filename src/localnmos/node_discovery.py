...
from .nmos import NMOS_Node, NMOS_Device, InputDevice, InputChannel, OutputDevice, OutputChannel
...
async def register_node_from_health_update(
    ...
    async with aiohttp.ClientSession() as session:
        ...
        logger.info(f"About to fetch senders for {node_id}")
        try:
            ...
            async with session.get(senders_url, timeout=aiohttp.ClientTimeout(total=5)) as response:
                if response.status == 200:
                    ...
                    # Note: Sender details would be processed here if needed
                else:
        ...
        try:
            ...
            async with session.get(receivers_url, timeout=aiohttp.ClientTimeout(total=5)) as response:
                if response.status == 200:
                    ...
                    # Note: Receiver details would be processed here if needed
                else:
        ...
        try:
            ...
            async with session.get(sources_url, timeout=aiohttp.ClientTimeout(total=5)) as response:
                if response.status == 200:
                    ...
                    # Note: Source details would be processed here if needed
                else:
